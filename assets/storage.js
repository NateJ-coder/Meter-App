...
export const storage = {
    ...
    getCycles(schemeId = null) {
        ...
        // Filter out pipeline-created cycles — these were generated by the historical
        // import (utility_dash_macrofree) and must never appear in the operational UI.
        // Real cycles always span a date range (start_date !== end_date).
        const operationalCycles = cycles.filter(c =>
            String(c.imported_from || '').toLowerCase() !== 'utility_dash_macrofree' &&
            !(c.start_date && c.end_date && c.start_date === c.end_date)
        );
        return schemeId ? operationalCycles.filter(c => c.scheme_id === schemeId) : operationalCycles;
    },
